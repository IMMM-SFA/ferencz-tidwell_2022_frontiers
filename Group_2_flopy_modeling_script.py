...
os.chdir(experiment_path + '/Group_2') 
model_path = experiment_path + '/Group_2/MODFLOW_files'

model_name = ['Geom_500m', 'Geom_1000m', 'Geom_2000m'] # number of models, outer loop of iterative simulation script 
params = pd.read_csv('Group_2_params.csv') # paramater settings file for group of simulations 
reset_params = pd.read_csv('Group_2_reset.csv') # parameters to reset MODFLOW models to default parameter values
# params = pd.read_csv('Modflow_scripts/Input_Files/Group_2/Group_2_params.csv') # paramater settings file for group of simulations 
# reset_params = pd.read_csv('Modflow_scripts/Input_Files/Group_2/Group_2_reset.csv')
...
baseline_results = np.zeros([144,12*365+1])
combinations = 48 # number of parameter combinations for each model

## Return flow simulations for Group 2 (irrigation recharge = ON)

for i in range(3): # number of MODFLOW models 
    for j in range(combinations): # number of iterations for each model 
        sim_name = model_name[i] # number of parameter combinations for each model 
        sim_path = model_path  + '/' + model_name[i]
        #sim_path = 'C:/Users/sbferen/Documents/Frontiers_Paper/Modflow_scripts/MODFLOW_models/Group_2/' + model_name[i] 
        os.chdir(sim_path)
        ...
        # write simulation to a new folder
...
for i in range(3):
    ...
    sim_path = sim_path = model_path  + '/' + model_name[i]
    #sim_path = 'C:/Users/sbferen/Documents/Frontiers_Paper/Modflow_scripts/MODFLOW_models/Group_2/' + model_name[i] 
    os.chdir(sim_path)
    sim = flopy.mf6.MFSimulation.load(sim_name=sim_name, version= 'mf6', exe_name = experiment_path + '/mf6.exe', 
                                  sim_ws=sim_path)
...
## Baseline simulations simulations for Group 2 (irrigation recharge = OFF)
...
return_flow_results.to_csv("Group_2_return_flow_results.csv")
baseline_results.to_csv("Group_2_baseline_results.csv")
